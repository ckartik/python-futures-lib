...
def future(func, paramList):
    return cf.ProcessPoolExecutor(max_workers=len(tags)).map(func, paramList)
...
urls = ['www.ckartik.com', 'http://ckartik.com/posts/os-concepts-common/', 'http://ckartik.com/about/', 'http://ckartik.com/posts/exception-handling/']
start = time.time()
posts = [post for post in query_future(query, urls)]
end = time.time()
